...
import { ensureUserEmail, promptForEmail, setUserEmail as setSharedUserEmail } from '../shared/currentUser'
...
onMounted(() => {
  ...
  // Email storage/prompt now lives in shared/currentUser.ts (so the Home
  // page's topbar can read the same signed-in email) — these wrappers just
  // keep the local USER_EMAIL closure var and topbar DOM text in sync with
  // that shared, reactive source of truth.
